    ...
    request("http://ciy-bumpsandcrashes-api.herokuapp.com/crashes?apikey=" + BCapikey + "&keywordtype=" + req.query.keywordtype + "&keyword=" + req.query.keyword, function(err2, response2, body2) {
      if (!err && !err2) {
        ...
        res.send(JSON.stringify({
          bumps: JSON.parse(body),
          crashes: JSON.parse(body2)
        }));
      } else {

        res.send(JSON.stringify({
          errmessage: 'BC API returned no data'
        }));
      }
...
router.post('/savedata', function(req, res) {
  ...
  BC.addBCData(req.body.id, req.body.savename, {
    "keyword": req.body.keyword,
    "keywordtype": req.body.keywordtype
  }, function() {
